...
public class RangeFacetBuilderHelper extends RangeFilterBuilderHelper implements IFacetBuilderHelper {
    ...
    public RangeFacetBuilderHelper(final String esFieldName, final RangeFacet rangeFacetAnnotation) {
        super(esFieldName, rangeFacetAnnotation.ranges());
        this.ranges = rangeFacetAnnotation.ranges();
    }

    @Override
    public FacetBuilder buildFacet() {
        RangeFacetBuilder rangeFacetBuilder = new RangeFacetBuilder(getEsFieldName()).field(getEsFieldName()).addUnboundedFrom(
                this.ranges[0]);
        int i = 1;
