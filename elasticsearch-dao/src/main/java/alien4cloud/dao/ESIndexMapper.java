...
public abstract class ESIndexMapper {
    ...
    public void initIndices(String indexName, String ttl, Class<?>... classes) {
        if (indexExist(indexName)) {
            ...
            for (Class<?> clazz : classes) {
                ...
                JavaType mapStringObjectType = jsonMapper.getTypeFactory().constructParametricType(HashMap.class, String.class, Object.class);
                Map<String, Object> typesMap = jsonMapper.readValue(typeMapping, mapStringObjectType);
            ...
            if (!createResponse.isAcknowledged()) {
                throw new ElasticSearchDAOException("Failed to create index <" + indexName + ">");
            }
        }
    }
    ...
    public String getIndexForType(Class<?> clazz) {
        ...
        if (index == null) {
            ...
            throw new ElasticSearchDAOException("Requested type <" + typeName + "> is not registered in any indexes.");
        }
        return index;
    }
     ...
     *            false then an {@link ElasticSearchDAOException} is thrown.
    ...
    public Class<?> getClassFromName(String className, boolean allowNull) {
        if (className == null || className.trim().isEmpty()) {
            ...
            throw new ElasticSearchDAOException("Class type <" + className + "> not found.");
        } else {
            try {
                return Class.forName(className);
            } catch (ClassNotFoundException e) {
                log.error("Error while trying to perform search operation.", e);
                throw new ElasticSearchDAOException("Class type <" + className + "> not found.", e);
            }
        }
    }
