...
public abstract class ESGenericIdDAO extends ESIndexMapper implements IGenericIdDAO {
    ...
    public <T> void save(T data) {
        ...
        String documentId = getMappingBuilder().getDocumentId(data);
        String routing = getMappingBuilder().getDocumentRouting(data);
        String timestamp = getMappingBuilder().getDocumentTimestamp(data);
        getClient().prepareIndex(indexName, typeName).setId(documentId).setRouting(routing).setTimestamp(timestamp).setSource(json).setRefresh(true).execute()
                .actionGet();
    }
    ...
    public <T> void save(T[] entities) {
        ...
        for (T data : entities) {
            ...
            String routing = getMappingBuilder().getDocumentRouting(data);
            String timestamp = getMappingBuilder().getDocumentTimestamp(data);
            bulkRequestBuilder.add(getClient().prepareIndex(indexName, typeName).setId(documentId).setRouting(routing).setTimestamp(timestamp).setSource(json));
        }
    ...
    private void assertIdNotNullFor(String id, String operation) {
        if (id == null || id.trim().isEmpty()) {
            ...
            throw new ElasticSearchDAOException("Null or empty Id is not allowed for operation <" + operation + ">.");
        }
    }
